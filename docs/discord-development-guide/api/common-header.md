...
Discord 메시지 전송처럼 되돌릴 수 없는 작업을 재시도가 중복 실행하지 않게 막는다.
네트워크 타임아웃 뒤 같은 요청을 다시 보내도 메시지가 두 번 올라가지 않아야 한다.

- 요청당 하나의 새 UUID v4. **재시도할 때는 같은 값을 그대로 다시 보낸다** (재시도마다 새로
  만들면 막을 수 있는 게 없다).
- 같은 키로 이미 처리된 요청이 오면 새로 실행하지 않고 **처음 응답을 그대로** 돌려준다.
- 같은 키에 다른 바디가 오면 **409**.
- 조회(`GET`)에는 붙이지 않는다.
