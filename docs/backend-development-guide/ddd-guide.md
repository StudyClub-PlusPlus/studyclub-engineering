...
- [조회 코드 배치 → query-placement-guide.md](query-placement-guide.md)
- [도메인 예외](#도메인-예외)
...
> 📌 **목록·검색처럼 여러 애그리거트를 읽는 조회는 애그리거트 리포지토리에 두지 않는다.** 자리와 판정 기준은
> [`query-placement-guide.md`](query-placement-guide.md). 페이지네이션이 있으면 필터·정렬도 DB 에서 한다 —
> 메모리에서 자르면 `total` 과 페이지가 어긋난다.
