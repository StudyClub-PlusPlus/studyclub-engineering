...
> 공개 여부는 [`STATUS != DRAFT`](#공개-여부) 하나로 정한다.
...
**단계 이름에 「공개」를 쓰지 않는다.** `DRAFT` 를 벗어난 네 단계(`OPEN`·`ONGOING`·`ENDED`·`CLOSED`) 는
전부 공개 상태라, "공개"라는 이름을 특정 단계 하나에 붙이면 나머지 세 단계도 공개라는 사실을 가린다.
공개 여부는 [`STATUS != DRAFT`](#공개-여부) 로, 신청 가능 여부는 [모집 상태](#모집-상태-계산--저장-안-함)로
따로 판정한다 — `STATUS` 값 자체는 "어디까지 진행됐는가"만 말하고, 공개·신청가능 여부는 그 값을 읽어
계산한다.
...
별도 플래그(`IS_HIDDEN`)는 없다. **판정은 `STUDY.STATUS != DRAFT` 하나다.** `DRAFT` 만 비공개이고
`OPEN`·`ONGOING`·`ENDED`·`CLOSED` 는 전부 공개다(사용자 사이트 표기는 위 표대로 다시 묶는다).

`STUDY_RECRUITMENT.START_AT`(모집 시작 일자)로 판정하지 않는다 — 캡틴이 모집을 시작하면
`STATUS: DRAFT → OPEN` 과 `START_AT` 채움이 **항상 같이** 일어나 두 값이 이론상 늘 일치하지만, 필드가
둘이면 한쪽만 바뀌는 버그가 생길 여지가 있다(예: 공개 취소가 `STATUS` 는 `DRAFT` 로 되돌리고 `START_AT`
은 못 지우면, `START_AT` 기준 판정은 여전히 "공개"로 남는다). `STATUS` 하나만 보면 이 동기화 문제 자체가
없다. `START_AT` 은 "모집이 언제 시작했는가"를 말하는 사실 데이터로만 쓰고, 공개 여부의 근거로 쓰지
않는다.

> **`START_AT=now` 등록 버그는 별도로 고쳐야 한다**: 지금 등록 API는 `STUDY_RECRUITMENT.START_AT` 을
> `now()` 로 채운다 — 등록 직후 미공개로 두려는 기획과 어긋나는 기존 동작이다(아래 "왜 고쳤나" 표 참고).
> 공개 판정을 `STATUS` 기준으로 바꾸면 이 값이 공개 여부에 더는 영향을 주지 않아 **노출 사고 자체는
> 막힌다** — `STATUS` 판정으로 먼저 옮기고 등록 로직은 뒤이어 고쳐도 안전하다. 다만 `START_AT` 은
> "모집이 실제로 시작한 날"이라는 사실 데이터이므로, `now()` 를 채우는 등록 로직은 데이터 정확성을 위해
> 여전히 고쳐야 한다 — 공개 판정과는 별개의 버그다.
