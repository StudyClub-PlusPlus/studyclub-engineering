...
어떻게 운영되는가"는 여기가 답한다. 클럽(`TYPE=RECURRING`)은 여러 코호트를 갖고,
지난 기수는 그대로 남아 이력 조회가 가능해야 한다. 일회성(`ONE_TIME`)·상시(`ROLLING`)
스터디도 예외 없이 코호트를 1개 갖는다 — "코호트 없는 STUDY"라는
특수 케이스를 만들지 않는다.
...
| FORMAT | VARCHAR(20) | N | `ONLINE` / `OFFLINE` / `HYBRID` |
| STATUS | VARCHAR(20) | N | 아래 |
| FORM | JSON | Y | 이 기수 신청 폼 질문 정의 |
| CURRICULUM | JSON | Y | 주차별 커리큘럼. 구조는 프론트와 합의 |
| CAPACITY | INT | Y | 이 기수 전체 정원. 반별 정원은 STUDY_CLASS |
| DEADLINE | DATETIME | N | 이 기수 모집 마감 |
| START_DATE | DATE | Y | 진행 시작일 |
| END_DATE | DATE | Y | 진행 종료일. **ROLLING 코호트는 NULL** — 고정 종료 없음 |
| DISCORD_CHANNEL | VARCHAR(512) | Y | 이 기수 디스코드 채널 링크 |
...
| `RECRUITING` 모집중 | `모집 시작 <= now() < DEADLINE` 그리고 정원 미달. `TYPE=ROLLING` 이면 `DEADLINE` 무시 |
| `RECRUIT_CLOSED` 모집마감 | `now() >= DEADLINE` 또는 정원 도달 (ROLLING 은 정원 도달 시에만) |
| `ONGOING` 진행중 | `START_DATE <= today <= END_DATE` (ROLLING 은 `END_DATE IS NULL` 이므로 `START_DATE` 이후 항상 참) |
| `ENDED` 종료 | `today > END_DATE` (ROLLING 은 해당 없음) |

## 제약
- 인덱스 `(STATUS, DEADLINE)` — 탐색 목록 쿼리 (구 STUDY 인덱스 이동)
