...
> STUDY_DELIVERY_FORMAT · STATUS · APPLICATION_FORM · CURRICULUM · CAPACITY · RECRUIT_DEADLINE ·
> START_DATE/END_DATE · DISCORD_CHANNEL_URL · DRIVE_URL 은 기수마다 달라질 수 있고
> 지난 기수 값은 보존돼야 하므로 전부 [STUDY](./STUDY.md) 로 이동했다.
...
| 값 | 뜻 |
|---|---|
| `STUDY` | 스터디. 한 번 모집해 한 번 진행. STUDY 를 딱 1개만 갖는다 |
| `CLUB` | 클럽. 기수제로 반복 — STUDY 가 여러 개 쌓인다 |

스터디가 클럽이 되면 `STUDY → CLUB` 으로 바꾼다.

## 제약
- `UNIQUE(SLUG)` — `uk_study_program_slug`

> 탐색 목록 쿼리용 인덱스(`STATUS`, `RECRUIT_DEADLINE` 기준)는 해당 컬럼들과 함께
> [STUDY](./STUDY.md#제약) 로 이동했다.
