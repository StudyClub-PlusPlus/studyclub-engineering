...
import { RegionClocks } from '@core/components/RegionClocks';
import { getSite, getOperators, type Locale } from '@core/lib/content';
...
export default async function AboutPage({ params }: { params: Promise<{ locale: Locale }> }) {
  ...
  const captainPoints = [
    {
      ko: '스터디 초기 세팅(모집 광고 · 디스코드 · 킥오프)을 도와줍니다.',
      en: 'They handle the initial setup — recruiting, Discord, and kickoff.',
    },
    {
      ko: '100% 자원봉사이며, 수익은 전액 기부합니다.',
      en: '100% volunteer, and all profits are donated.',
    },
    {
      ko: '캡틴은 모든 스터디와 이벤트에 무료로 참여합니다.',
      en: 'Captains join every study and event for free.',
    },
  ...
  const cards = [
    {
      ...
      icon: BookOpen,
      label: m('studies.title', locale),
      external: false,
    },
    {
      href: `/proto/core/${locale}/events`,
      icon: Calendar,
      label: m('events.title', locale),
      external: false,
    },
    ...(site.mentoring_url
      ? [
