...
export default function MyStudiesPage() {
  ...
  useEffect(() => {
    if (!getUser()) {
      router.replace(`/proto/core/${locale}/login?next=/proto/core/${locale}/my/studies`);
      return;
    }
    setMineIds(
      getApplications()
  ...
  return (
    <div className='mx-auto max-w-3xl px-6 pb-16 pt-8'>
      ...
      <nav data-anno='2' className='mt-5 inline-flex rounded-pill bg-surface-2 p-1'>
        {(
          [
...
function TodayCard({ study, locale, onChange }: { study: Study; locale: Locale; onChange: () => void }) {
  ...
  return (
    <li data-anno='3' className='card flex items-center gap-4 px-5 py-4'>
      <span
      ...
      <div className='min-w-0 flex-1'>
        ...
        <p
          data-anno='3-2'
          className='mt-0.5 flex items-center gap-1.5 truncate text-[13px] text-fg-secondary'
        >
      ...
      {session &&
        (status ? (
          <span
            data-anno='3-3'
            className={`shrink-0 rounded-pill px-3 py-1.5 text-xs font-bold ${STATUS_STYLE[status]}`}
          >
            {STATUS_LABEL[status]}
          </span>
        ) : (
  ...
  return (
    <section data-anno='4' className='card px-5 py-4'>
      <div className='flex items-baseline justify-between gap-3'>
        ...
        <span data-anno='4-1' className='tnum shrink-0 text-sm font-bold text-fg'>
          {rate === undefined ? <span className='text-fg-muted'>—</span> : `${rate}%`}
        </span>
      </div>

      <div data-anno='4-2' className='no-scrollbar mt-3 overflow-x-auto'>
