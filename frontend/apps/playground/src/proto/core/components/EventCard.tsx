...
export function EventRow({ event, locale }: { event: StudyclubEvent; locale: Locale }) {
  ...
  return (
    <Link
      href={`/proto/core/${locale}/events/${event.id}`}
      className='group flex items-center gap-4 px-4 py-3.5 transition-colors hover:bg-(--color-surface-subtle) sm:px-5'
    >
      {/* 날짜 박스 */}
      <div
      ...
      <div className='min-w-0 flex-1'>
        ...
        <p className='truncate text-[13px] text-(--color-fg-muted)'>{t(event.summary, locale)}</p>
      </div>

      <ChevronRight
        size={18}
        className='shrink-0 text-(--color-fg-faint) transition-transform group-hover:translate-x-0.5'
      />
    </Link>
  );
}
