...
export function BookmarkButton({
  ...
  const label = t(
    marked ? { ko: '관심 스터디에서 빼기', en: 'Remove from saved' } : { ko: '관심 스터디에 담기', en: 'Save study' },
    locale,
  );
  ...
  return (
    <button
      ...
      aria-label={label}
      title={label}
      className={`relative z-2 inline-grid h-9 w-9 shrink-0 place-items-center rounded-full transition-[background-color,transform] hover:scale-110 focus-visible:outline-none focus-visible:shadow-(--ring) ${
        tone === 'onColor'
          ? ready && marked
    ...
    >
      ...
      */}
      <Heart
        ...
        strokeWidth={2}
        className={
          ready && marked ? 'fill-current text-error-600' : tone === 'onColor' ? 'text-white/90' : 'text-fg-placeholder'
        }
      />
    </button>
  );
