...
기준 프로토타입: 스터디 신청 (playground 사용자 사이트 · 상세의 신청하기)

> 갱신: 2026-09-22 — 신청하기·찜은 목록에서 뺐다. 목록은 둘러보기 전용, 신청은 상세에서만 한다. [crew-browse-studies](../crew-browse-studies/PRD.md) 참고
...
1. 모집중 스터디의 상세에서 신청 폼을 열 수 있다. 목록에는 신청하기를 두지 않는다 — 둘러보기와 신청을 화면으로 나눈다
...
7. 완료 화면에서 지금 화면(상세)에 머물거나, 내 스터디에서 방금 낸 기수가 있는 탭을 연다
...
- **정책**
  ...
  - 재시도가 `409` 이미 신청이면 완료 화면으로 바꾼다. `409` 모집 마감·정원 초과면 폼을 닫고 상세 CTA를 그 상태로 바꾼다
...
- **내용**: `취소`
- **동작**: 연동하지 않고 닫는다. 신청 폼은 열리지 않는다. 상세에 머문다
- **정책**
  - 연동 처리 중에는 누를 수 없다
...
| 상태 | 화면 | 카피 |
| --- | --- | --- |
| 미로그인 CTA | 상세 | `신청하기`. 누르면 로그인. `next` = 이 신청 흐름 |
| 모집중 CTA | 상세 | `신청하기` |
...
| 이미 신청 | 상세 CTA | `신청 완료`. 폼 없음. 수정·삭제 없음 |
| 모집 마감 | 상세 CTA | `모집 마감`. 폼 없음 |
| 정원 마감 | 상세 CTA | `정원 마감`. 폼 없음 |
...
- 구현 지시: 신청하기는 상세에만 둔다. 목록은 둘러보기·필터링 전용이며 신청하기·찜을 두지 않는다
...
- 타인 신청: 상세·이 폼에서 다른 사람의 답·별명을 보여 주지 않는다
