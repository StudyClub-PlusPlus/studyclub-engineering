...
- 분야(카테고리)로 걸러 봄
- 카드 상단 배지로 모집 상태(모집중 D-N·마감 임박·모집중(미정)·진행중·모집 마감)를 한눈에 확인
- 카드에서 시간대·시작 예정일을 확인하고, 카드를 눌러 상세로 이동해 신청
...
- **정책**
  - 기본값은 전체
  - **`study.timezone` 이 있으면 그 값을 그대로 쓴다** — 운영 콘솔 등록 폼에서 캡틴이 직접 고른 값이라 가장 정확하다(2026-09-23 추가)
  - 이 필드가 없는(생기기 전) 옛 데이터만 일정(`study.schedule`)·킥오프(`study.recruitment.kickoff`) 문구에서 `KST`/`PST`·`PDT` 표기를 읽어 추정한다
  - 그마저도 표기가 없으면 `동시 모집` 버킷으로 분류한다 — 이 버킷은 필터 전용 이름이다. "동시에 진행한다"는 뜻이 아니라 "모른다"는 뜻이라, 카드 문구(5-1)는 이 경우 정직하게 `시간대 미정`이라고 쓴다
- **데이터**: `study.timezone`, `study.schedule`, `study.recruitment.kickoff`
...
- **동작**: 카드를 누르면 해당 스터디 상세로 이동한다
- **정책**
  - 신청하기 버튼·찜(하트) 버튼은 이 카드에 두지 않는다 — 신청·찜은 상세에서만 한다
  - 모집 상태 배지: 모집 중이고 마감일이 있으면 `모집중 (D-N)`(`D-DAY`는 당일), 마감일이 없으면 `모집중 (미정)`(마감일은 필수라 옛 데이터에만 있을 수 있다), 진행 중이면 `진행중`, 끝났으면 `모집 마감`
  ...
  - 모집 마감일은 상단 배지가 이미 말하므로(D-N·모집 마감·모집중(미정)과 항상 겹친다) 본문에 따로 두지 않는다
  - 시간대: `study.timezone` 이 있으면 `KST`/`PST`/`동시 진행(KST·PST)`을 그대로 쓴다. 없으면(3. 시간대 필터와 같은 기준으로 추정하되) 명시된 표기조차 없으면 `시간대 미정` — `동시 모집`이라 단정하지 않는다
- **데이터**: `study`
- **조건**: 필터 결과가 1개 이상일 때
...
- 상세 페이지도 같은 상태 배지·시작 예정일 표기를 쓴다(`recruitBadge`·`studyStartValue` 공용) — 목록·상세 표기가 어긋나지 않도록 화면마다 다시 계산하지 않는다. 상세의 신청 흐름 자체는 [crew-submit-application](../crew-submit-application/PRD.md) 참고
...
- 상태 배지: `recruitBadge(study, locale)` — 문구(`모집중 (D-N)`·`모집중 (미정)`·`진행중`·`모집 마감`)와 점 색(tone)을 함께 반환. 마감임박(`D-3` 이하) 판정은 백엔드 `isClosingSoon()`과 같은 기준(3일) — [study-recruit-status/spec.md](../../../specs/study-recruit-status/spec.md#판정-규칙). 목록 카드·상세가 같은 함수를 쓴다
- 시작일 표시값: `studyStartValue(study, locale)` — `STUDY.START_AT` 없으면 `미정`. 목록 카드·상세가 같은 함수를 쓴다
- 시간대 분류: `studyTimezone(study)` — `study.timezone`(등록 폼에서 직접 고른 값)이 있으면 그대로, 없으면(옛 데이터) 일정·킥오프 문구의 `PST`/`PDT`·`KST` 표기로 판정. 그마저 없으면 `both`(필터 버킷 이름은 「동시 모집」이지만 실제 뜻은 "모른다"). 목록 필터·카드·상세가 같은 함수를 쓴다. 화면 문구는 `studyTimezoneLabel()` 이 따로 계산한다 — `both` 를 「동시 모집」이라 단정해 보여주지 않고 「시간대 미정」으로 정직하게 표시한다
