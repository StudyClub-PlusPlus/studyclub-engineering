...
--   * ACCOUNT: ONBOARDING_COMPLETED_AT, NICKNAME VARCHAR(20), UNIQUE(NICKNAME),
--              uk_users_* → uk_account_* 제약 이름 통일
--   * ACCOUNT_IDENTITY: PROVIDER_USER_ID → PROVIDER_ACCOUNT_ID, PROVIDER_EMAIL NOT NULL
...
-- ACCOUNT
-- ---------------------------------------------------------------------------
-- 기존 255 컬럼에 긴 값이 있으면 축소 전에 잘라 둔다 (로컬·누적 DB 대비).
UPDATE ACCOUNT SET NICKNAME = LEFT(NICKNAME, 20) WHERE CHAR_LENGTH(NICKNAME) > 20;
...
-- 테이블명이 ACCOUNT 로 바뀐 뒤에도 남아 있던 uk_users_* 이름을 맞춘다.
ALTER TABLE ACCOUNT RENAME INDEX uk_users_email TO uk_account_email;
ALTER TABLE ACCOUNT RENAME INDEX uk_users_discord_id TO uk_account_discord_id;
