...
/**
 * 닉네임 형식 규칙 (specs/user-onboarding/spec.md). 온보딩뿐 아니라 마이페이지 닉네임 변경
 * 등 나중에 생길 다른 진입점에서도 같은 규칙을 쓸 수 있도록 순수 도메인 규칙으로 둔다 — 웹 계층
 * 의존 없음. 중복 검사(DB 조회가 필요한 상태 의존 규칙)는 여기 없다 — {@code AccountRepository}
 * 를 쓰는 호출자의 책임이다.
 */
public final class NicknamePolicy {
    ...
    /**
     * 형식이 유효하면 {@code null}, 아니면 사용자에게 보여줄 위반 사유.
     * 호출 전 {@link #normalize} 를 거친 값을 넘긴다.
     */
