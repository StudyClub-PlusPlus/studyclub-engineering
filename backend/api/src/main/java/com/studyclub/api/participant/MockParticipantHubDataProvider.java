...
public class MockParticipantHubDataProvider implements ParticipantHubDataProvider {
    ...
    public ParticipantHubOverviewResponse getParticipantHubOverview(String accountEmail) {
        ...
        return new ParticipantHubOverviewResponse(
                List.of(
                        new ParticipatingStudySummary(301L, 101L, "AI 논문 읽기", ParticipantStatus.ACTIVE, 100,
                                Instant.parse("2026-09-08T11:00:00Z"), "https://example.com/studies/101.png"),
                        new ParticipatingStudySummary(302L, 102L, "Spring Boot 딥다이브", ParticipantStatus.ACTIVE, null,
                                Instant.parse("2026-09-12T10:00:00Z"), "https://example.com/studies/102.png")),
                List.of(
                        new ParticipatingStudySummary(291L, 91L, "개발자 글쓰기", ParticipantStatus.COMPLETED, 88,
                                null, "https://example.com/studies/91.png")),
                List.of(
                        new StudyApplicationSummary(501L, 303L, 103L, "오픈소스 첫 기여", ApplicationStatus.PENDING,
                                Instant.parse("2026-09-05T04:30:00Z")),
                        new StudyApplicationSummary(502L, 304L, 104L, "데이터 시각화", ApplicationStatus.REJECTED,
    ...
    private ParticipatingStudyCohortDetailResponse ongoingStudy() {
        return new ParticipatingStudyCohortDetailResponse(
                301L, 101L, "AI 논문 읽기", StudyCohortStatus.OPEN, ParticipantStatus.ACTIVE,
                "화요일반", "Asia/Seoul", "김○○",
                3, 12, 100, java.time.LocalDate.parse("2026-08-18"),
                new UpcomingStudyMeeting(1004L, 301L, 101L, "AI 논문 읽기",
                        Instant.parse("2026-09-08T11:00:00Z")),
                List.of(
                        completedAttendance(1001L, "2026-08-18T11:00:00Z", AttendanceStatus.PRESENT),
                        completedAttendance(1002L, "2026-08-25T11:00:00Z", AttendanceStatus.PRESENT),
                        completedAttendance(1003L, "2026-09-01T11:00:00Z", AttendanceStatus.LATE),
    ...
    private ParticipatingStudyCohortDetailResponse upcomingStudy() {
        return new ParticipatingStudyCohortDetailResponse(
                302L, 102L, "Spring Boot 딥다이브", StudyCohortStatus.OPEN, ParticipantStatus.ACTIVE,
                "토요일반", "Asia/Seoul", "박○○",
    ...
    private ParticipatingStudyCohortDetailResponse completedStudy() {
        return new ParticipatingStudyCohortDetailResponse(
                291L, 91L, "개발자 글쓰기", StudyCohortStatus.CLOSED, ParticipantStatus.COMPLETED,
                "수요일반", "Asia/Seoul", "이○○",
                8, 8, 88, java.time.LocalDate.parse("2026-05-06"), null,
                List.of(
                        completedAttendance(901L, "2026-05-06T11:00:00Z", AttendanceStatus.PRESENT),
                        completedAttendance(902L, "2026-05-13T11:00:00Z", AttendanceStatus.PRESENT),
                        completedAttendance(903L, "2026-05-20T11:00:00Z", AttendanceStatus.PRESENT),
                        completedAttendance(904L, "2026-05-27T11:00:00Z", AttendanceStatus.PRESENT),
                        completedAttendance(905L, "2026-06-03T11:00:00Z", AttendanceStatus.PRESENT),
                        completedAttendance(906L, "2026-06-10T11:00:00Z", AttendanceStatus.PRESENT),
                        completedAttendance(907L, "2026-06-17T11:00:00Z", AttendanceStatus.PRESENT),
                        completedAttendance(908L, "2026-06-24T11:00:00Z", AttendanceStatus.ABSENT)),
                "https://drive.google.com/drive/folders/mock-writing-cohort-1");
    }

    private StudyMeetingAttendance completedAttendance(Long meetingId, String scheduledAt, AttendanceStatus status) {
