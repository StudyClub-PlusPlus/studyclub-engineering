...
public final class BackOfficeApplicationResponses {
    ...
    public record ApplicationQuestionResponse(
            ...
            @Schema(description = "질문 타입", example = "TEXT", allowableValues = {
    ...
    public record StudyApplicationResponse(
            ...
            @Schema(description = "신청자 계정 이메일", example = "gildong@example.com")
                    String email,
            ...
            @Schema(description = "이전 참여 중 COMPLETED 비율. 이전 참여가 없으면 null이며 화면에서는 첫 참여로 표시합니다.", example = "50")
            ...
            @Schema(description = "추가 질문 답변. 값은 문자열 또는 문자열 배열입니다.")
