...
public class StudyBookmarkService {
    ...
    public Result getBookmarks(Long accountId, int offset, int limit) {
        List<StudyBookmarkDtos> mockItems =
                List.of(
                        new StudyBookmarkDtos(
                                "java study", "OPEN", "BACKEND", "every week", "8 weeks"),
                        new StudyBookmarkDtos("DDIA", "CLOSED", "DATA", "once a week", "12 weeks"));
        return new Result(
                new StudyBookmarkResponse(mockItems, mockItems.size(), offset, limit),
                HttpStatus.OK);
    }
