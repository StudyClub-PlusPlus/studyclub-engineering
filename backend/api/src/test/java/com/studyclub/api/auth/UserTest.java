...
class UserTest {
    ...
    void dropsOverlongPicture() {
        ...
        User user = new User("a@b.com", "홍길동", tooLong, SystemRole.MEMBER);
    ...
    void keepsNormalPicture() {
        ...
        assertThat(new User("a@b.com", "n", ok, SystemRole.MEMBER).getProfileImgUrl()).isEqualTo(ok);
    }

    @Test
    @DisplayName("닉네임이 255 를 넘으면 잘라서라도 남긴다 — 잘린 이름도 사람을 알아보는 데 쓸모가 있다")
    void clipsOverlongNickname() {
        User user = new User("a@b.com", repeat('가', 300), null, SystemRole.MEMBER);

        assertThat(user.getNickname()).hasSize(255);
    }

    @Test
    @DisplayName("null 은 그대��� 통과 — 사진 없는 계정이 있다")
    void allowsNull() {
        User user = new User("a@b.com", null, null, SystemRole.MEMBER);
    ...
    void settersGuardToo() {
        User user = new User("a@b.com", "n", null, SystemRole.MEMBER);

        user.setProfileImgUrl("https://x/" + repeat('y', 2100));
        user.setNickname(repeat('나', 300));
