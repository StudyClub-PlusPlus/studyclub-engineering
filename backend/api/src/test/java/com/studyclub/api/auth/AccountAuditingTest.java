 ...
 *
 * <p>이건 순수 단위 테스트로 잡히지 않는다 — {@code @CreatedDate} 는 어노테이션일 뿐이고, 값을 채우는 건 {@code @EnableJpaAuditing}
 * 이 켜는 리스너다. 그 한 줄을 지우면 <b>컴파일도 되고 엔티티 단위 테스트도 통과하는데 DB 에는 null 이 들어간다.</b> 그래서 persist 까지 하는 슬라이스
 * 테스트로 둔다.
 */
@DataJpaTest
...
class AccountAuditingTest {
    ...
    void fillsAuditColumnsOnInsert() {
        ...
        Account saved =
                accounts.save(
                        new Account("audit@studyclub-plusplus.com", "감사", null, SystemRole.MEMBER));
        em.flush();
    ...
    void movesOnlyUpdatedAtOnModify() {
        ...
        Account saved =
                accounts.save(
                        new Account("edit@studyclub-plusplus.com", "수정", null, SystemRole.MEMBER));
        em.flush();
