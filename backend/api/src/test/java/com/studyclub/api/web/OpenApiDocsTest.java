 ...
 *
 * <p>이게 없으면 SecurityConfig 화이트리스트에서 한 줄만 빠져도 문서가 조용히 401 이 된다. 특히 {@code /webjars/**} 는 UI 가 아니라 UI
 * 가 로드하는 JS 라 빠져도 페이지 자체는 200 이고, 브라우저에서 빈 화면으로만 드러난다.
 */
...
class OpenApiDocsTest {
    ...
    void scalarUiAndItsBundleArePublic() {
        assertThat(rest.getForEntity("/scalar", String.class).getStatusCode())
