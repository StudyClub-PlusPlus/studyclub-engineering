-- V14 — 회원가입 웰컴메일 알림 (specs/notification/spec.md).
...
-- 모듈 경계 없이 한 시퀀스로 스캔한다. 이 PR 이 열려 있는 동안 domain 모듈에 V12(recruit_schema, PR #91)와
-- V13(drop_study_application_status)이 먼저 병합되어 두 차례 버전이 충돌했다(Flyway "Found more than one
-- migration with version N") — V14 로 재번호했다. 머지 직전에는 다시 한번 beta 의 최신 번호를 확인할 것.
