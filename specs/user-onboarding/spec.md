...
- `USER.EMAIL` 은 식별자가 아니라 **연락처**. 로그인 때 받은 걸 넣는다. 수정은 PRD 상 안 함.
...
- **온보딩 전 `NICKNAME` 을 NULL 로 허용하나 → 아니오. `NOT NULL` 유지.** 로그인 직후엔 `user_<랜덤>` 임시값을 넣고 온보딩에서 정한다. 애플은 이름을 첫 로그인에만 주니 제공자 이름에 기대면 안 되고, NULL 을 허용하면 명부·후기 등 닉네임을 보여주는 모든 곳이 NULL 을 따로 처리해야 한다. 임시값 + `UNIQUE(NICKNAME)` 이면 구글·애플 구분 없이 같은 흐름.
- **애플 비공개 중계 주소(`@privaterelay.appleid.com`)를 `USER.EMAIL` 로 인정하나 → 예. 그대로 저장.** 식별은 `sub` 라 이메일이 뭐든 로그인엔 영향 없고, 중계 주소도 애플이 실제 메일함으로 전달해 주니 연락처로 유효하다. 원본은 `IDENTITY.PROVIDER_EMAIL` 에 남는다. 단 구글 이메일과 달라 구글 계정이 있어도 안 묶인다 → 별개 USER, 명시적 연결(후속). 중계 주소로 메일을 보내려면 발신 도메인을 애플 개발자 계정에 등록해야 한다고 알고 있다 — **알림팀이 구현 전 확인**.
...
- **`USER.EMAIL` 수정** — PRD 대로 안 함. 허용하려면 `UNIQUE(EMAIL)` 충돌 + 변경 인증 메일이 따라온다.
