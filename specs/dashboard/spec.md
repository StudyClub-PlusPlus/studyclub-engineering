...
- **작성 기준 확정**: 기수 단위 집계, 활성 크루의 범위, 온보딩 완료 회원 수, 시간대를 거주지로 간주하는 가정과 한국·북미 분류 및 미분류 처리, 출석 계산 및 비교 기간, 마감 경과 표시, 진행 중 기수의 동시 모집 허용, 행사 MVP 제외, 일부 지표 실패 시 정상 지표 유지.
...
첨부한 화면은 구성·배치·번호를 설명하는 기준이다. 파란 번호와 점선은 설명용이며 실제 화면에 표시하지 않는다. 화면의 `143`, `65%`, 지역별 인원, 행사 1건 등은 예시 데이터다. 지역별 인원 합계처럼 서로 맞지 않는 숫자를 구현 기준으로 사용하지 않는다. 지역 범례는 아래 결정에 따라 ‘한국 KST’·‘북미 ET·PT’와 필요한 경우 ‘미분류’로 표시하고, 예정 행사는 MVP 결정에 따라 0건으로 표시한다.
...
- **`asOf`는 ‘이 시점을 기준으로 집계했다’는 뜻의 집계 기준 시각**이다. 숫자 0이 아니라 영어 대문자 O를 쓴다. 예를 들어 `asOf`가 한국 시간 9월 16일 15:00이면 해당 응답의 회원 수·출석률·모집 상태는 그 시점을 기준으로 계산한 값이다. 사용자 화면에는 필드명 대신 `2026-09-16 15:00 KST 기준`처럼 표시한다.
- 서버는 응답마다 `asOf`를 제공한다. 날짜·시각은 UTC ISO 8601로 전달하고 대시보드는 **`Asia/Seoul`**로 표시한다. 캡틴이 해외에 있어도 주간 통계 기준이 바뀌지 않는다.
...
- 하나의 API 응답 안에서는 같은 `asOf`를 사용한다. 서로 연결된 값인 활성 크루·참여 건수·1인당 참여 수도 같은 참여자 데이터를 기준으로 계산한다.
- **카드와 차트가 서로 다른 API로 조회되면 데이터를 읽는 순간이 조금 다를 수 있다.** 예를 들어 활성 크루 조회 당시에는 100명이었지만, 직후 새 참여자 1명이 등록되고 지역 분포를 조회하면 지역별 인원 합계는 101명일 수 있다. 모든 응답이 정확히 같은 순간의 DB 상태를 읽도록 보장하는 기능은 이 초안에 포함하지 않는다. 이를 기술적으로 ‘동일한 DB 스냅샷을 보장하지 않는다’고 표현한다. DB 스냅샷은 특정 순간의 데이터 상태를 뜻한다.
- 위 예시는 조회 사이에 실제 데이터가 바뀐 경우다. 같은 데이터를 집계하면서 중복 계산이나 누락으로 숫자가 어긋나는 것은 허용하지 않는다. 주가 바뀌는 순간에 출석 API들의 기준 주가 달라지면, 이번 주·전주 증감과 12주 차트를 함께 다시 조회해 비교 기간을 맞춘다.
...
- **`0`은 실제 집계 결과가 0**, **`null`은 계산할 유효 데이터가 없음**을 뜻한다. 집계·통신 오류는 어느 쪽으로도 바꾸지 않고 별도의 실패 상태로 구분한다.
- **특정 지표 집계에 실패해도 다른 정상 지표는 유지한다.** 같은 원천 데이터의 실패에 영향을 받는 지표는 함께 오류를 표시할 수 있다. 같은 API 응답에 포함되어 있다는 이유만으로 모든 지표를 실패 처리하지 않는다.
- 로딩 중에는 기존 카드 크기를 유지하는 스켈레톤을 표시한다. 최초 조회에서 실패한 카드에는 ‘정보를 불러오지 못했습니다.’와 ‘다시 시도’를 표시한다. 같은 응답 안의 정상 카드와 다른 성공 영역은 유지한다.
...
- 새로고침 전에는 시간이 흘렀다는 이유만으로 수치나 마감 문구를 따로 바꾸지 않는다. 마감 표시와 목록도 응답의 `asOf` 기준으로 맞춘다.
- 미정: 별도 정책 미정 없음. 일부 지표 실패 시 정상 지표를 유지하며, 이를 표현할 응답 필드의 최종 계약은 **TBD-07** 참조.

### 2. KPI 카드 공통

순서는 ‘활성 크루 → 평균 출석률 → 크루 1인당 참여 스터디 → 커뮤니티 멤버’다. 제목, 큰 숫자, 집계 범위 설명을 같은 비중으로 배치한다. 평균 출석률 카드 안에만 전주 대비 증감을 추가한다.

**결정된 오류 표시 규칙:** 특정 지표를 계산하지 못해도 정상적으로 조회된 KPI는 표시한다. 실패한 지표와 같은 원천 데이터의 실패에 영향을 받는 지표에만 오류를 표시한다.

| 상황 | 표시할 화면 |
|---|---|
| 출석 집계만 실패 | 평균 출석률·전주 대비에 오류 표시. 정상 조회된 나머지 KPI는 유지 |
| 활성 참여자 집계 실패 | 활성 크루와 이를 분모로 사용하는 1인당 참여 스터디 모두 오류 표시. 정상 조회된 출석률·커뮤니티 멤버는 유지 |
| 커뮤니티 멤버 집계만 실패 | 커뮤니티 멤버 카드에만 오류 표시. 정상 조회된 나머지 KPI는 유지 |
| 요약 API 요청 자체가 실패해 응답 데이터를 받지 못함 | KPI 네 개에 오류 표시. 별도 요청으로 정상 조회된 목록·차트는 유지 |

이전 조회 데이터가 있다면 공통 규칙에 따라 해당 카드의 이전 값·갱신 시각과 ‘갱신 실패 · 이전 데이터’를 표시한다. 이전 값이 없을 때 오류를 `0`이나 ‘기록 없음’으로 대신 표시하지 않는다. 인증·권한 오류는 공통 로그인 정책으로 처리하며, 일반적인 지표 집계 실패로 취급하지 않는다.

**구현 방향:** 요약 API 하나를 유지하면서 성공한 지표와 실패한 지표를 구분해 반환한다. 각 카드마다 API를 새로 만드는 것을 요구하지 않는다. ‘다시 시도’는 요약 API를 재요청할 수 있으며, 요청 중에도 기존 정상 카드는 유지한다. 새 응답을 적용할 때 활성 크루·참여 건수·1인당 참여 수는 같은 조회 결과로 함께 갱신한다.

**미정 — [NEEDS CLARIFICATION] TBD-07:** 위 화면 동작은 결정되었다. 성공·실패 상태를 표현하는 구체적인 필드 구조와 재시도 연결은 프론트·백엔드가 계약 검토에서 확정한다. 아래 API 절의 부분 성공 응답은 검토용 제안이다.
...
**‘진행 중’을 DB에서 확인하는 방법:** 활성 크루를 ‘진행 중인 기수의 ACTIVE 참여자’로 센다는 기준은 결정되었다. 남은 것은 개발자가 DB의 어떤 값을 보고 그 기수를 진행 중이라고 판단할지다. 초안 작성 시 확인한 코드의 저장 상태는 다음 세 가지였다.

| 저장 상태 | 뜻 | 진행 중 집계와의 관계 |
|---|---|---|
| `DRAFT` | 작성 중 | 작성 중이라는 이유로 진행 중에 포함하지 않음 |
| `OPEN` | 공개됨 | 공개됐다는 뜻만으로 실제 스터디 모임이 시작됐다고 볼 수 없음 |
| `CLOSED` | 해당 기수 종료 | 당시 상태 모델에서 종료된 기수 |

예를 들어 오늘 공개하고 다음 달에 시작하는 스터디도 `OPEN`일 수 있다. 따라서 공개 여부만 보고 진행 중에 포함하면 안 된다. 최종 설계에서 `ONGOING`을 별도 상태로 저장한다면 그 값을 조회하고, 시작일·종료일로 진행 여부를 판단한다면 정해진 날짜 규칙을 계산해야 한다.

여기서 **‘명부’는 해당 기수에 참여하는 회원 목록**, **‘명부 생성 시점’은 신청자가 그 목록에 등록되는 시점**을 뜻한다. 앞서 확인한 [PR #89의 변경안](https://github.com/StudyClub-PlusPlus/studyclub-engineering/blob/ab78e92219d27e3e977e653d8cda8e9c8d1d1f4c/docs/erd/STUDY_APPLICATION.md)은 신청 제출과 동시에 명부에 등록하는 방향이다. 실제 신청 기능이 그 방식으로 구현되면 생성된 명부를 조회한다. 신청자가 `ACTIVE`로 등록되더라도 해당 기수가 아직 진행 전이면 활성 크루에는 포함하지 않는다.

**미정 — [NEEDS CLARIFICATION] TBD-01:** 최종 상태 설계에서 진행 여부를 어떤 상태값 또는 날짜 조건으로 판단할지, 신청 기능의 실제 명부 등록 처리가 어떻게 연결되는지 확인해야 한다. **기수의 진행과 모집이 동시에 가능하다는 점은 결정되었으며 미정이 아니다.** 집계 기준을 다시 정하는 것이 아니라, 정한 기준을 실제 DB·API에 연결하는 작업이 남아 있다.
...
- **‘진행중 스터디 / 모집중 스터디 / 예정 행사’를 한 화면에서 동시에 볼 수 있게 배치한다.** 기준 화면처럼 세 영역이 나란히 보인다. 예를 들어 ‘모집중’을 눌러야 모집 목록이 보이고 진행중 목록은 숨겨지는 탭 방식으로 만들지 않는다.
- 제목 옆 숫자는 **해당 조건에 맞는 전체 건수**이고, 카드 안에는 그중 최대 4건만 미리 보여준다. 예를 들어 진행중 기수가 15개면 제목에 15, 목록에 4개, 하단에 ‘외 11개 스터디’를 표시한다. 이 하단 영역의 설명 번호가 **3-4**다. 0건이어도 카드 영역의 높이를 유지한다.
...
- **확정: 스터디를 진행하면서 새 참여자를 모집할 수 있다.** 진행 여부는 ‘지금 스터디 모임이 진행 중인가’, 모집 여부는 ‘지금 새 참여자를 받고 있는가’를 각각 판단한다. 두 조건을 동시에 만족하면 같은 기수를 진행중·모집중 두 영역에 모두 표시하고, 각 영역의 전체 건수에도 1개씩 포함한다.
- 예를 들어 ‘얼리버드 3기’가 이미 시작했고 추가 신청도 받고 있다면 진행중 목록과 모집중 목록 양쪽의 대상이다. 각 카드의 최대 4건 미리보기는 정렬 순서에 따라 표시하므로 한쪽 미리보기에서 보이지 않더라도 전체 대상에서는 제외하지 않는다. 두 영역의 건수를 더한 값을 전체 고유 기수 수로 사용하지 않는다.
- **미정 — [NEEDS CLARIFICATION] TBD-01:** 동시 진행·모집 허용 여부는 더 이상 미정이 아니다. 두 조건을 최종 상태 모델의 어떤 값으로 판단할지만 맞춰야 한다. 공개 상태 `OPEN`만으로 진행 중이라고 판정하지 않는다.

### 3-2. 모집중 스터디

아래의 날짜·D-day는 표시 형식을 설명하는 예시이며, 실제 값은 조회 결과와 집계 기준 시각으로 계산한다.

새 참여자를 모집하는 기수가 대상이다. **이미 진행 중이라는 이유로 모집 목록에서 제외하지 않는다.** 모집 조건을 만족하면 진행중 목록과 모집중 목록에 함께 포함한다. 모집이 끝난 기수를 ‘마감 경과’로 얼마나 남길지는 아래 TBD-02에서 별도로 정한다.

카드에는 최대 4개 행을 표시하고 날짜·남은 일수는 줄바꿈하지 않는다. 일반 모집은 기본 글자색으로 안내하며 별도의 배경색 배지를 붙이지 않는다. 마감 상태를 강조할 때도 같은 오른쪽 텍스트 영역을 사용한다. 마감일은 KST `MM-DD`, 정확한 시각은 툴팁·접근 가능한 설명에 `YYYY-MM-DD HH:mm KST`로 제공하는 안이다. 마감일이 없고 수동 마감도 아니면 오른쪽에 ‘상시’만 표시한다.

**표시 규칙 — 위에서부터 먼저 해당하는 조건 적용**

아래는 한국 시간 2026-09-16을 기준으로 든 예시다. 일반 모집·임박·오늘 마감·마감 경과·상시 모집 모두 같은 오른쪽 텍스트 영역에 아래 문구를 적용한다.

| 우선순위 | 조건 | 표시 예 | 의미 |
|---|---|---|---|
| 1 | 수동 모집 마감이 확인되었거나 `asOf >= recruitDeadline` | `~09-15 (마감 경과)` / 날짜가 없으면 `마감 경과` | 마감 시각이 지났거나 사람이 먼저 모집을 닫음. 수동 마감이면 사유도 설명에 제공 |
...
남은 일수는 KST 달력 날짜의 차이로 계산하는 안이다. 예를 들어 오늘 23:00이고 내일 01:00 마감이면 `D-1`이다. 임박은 주의 색상과 ‘임박’, 오늘 마감·마감 경과는 경고 색상과 각각의 문구를 함께 사용한다. **색을 보지 않아도 텍스트로 상태를 구분할 수 있어야 한다.** 일반 모집은 `~MM-DD (D-n)` 형식을 사용한다.
...
- **3-4는 설명용 화면 번호**이며 실제로 클릭하는 영역의 문구는 ‘외 N개 스터디’다.
- `전체 대상 건수 > 표시 건수`일 때만 `외 N개 스터디`를 표시한다. `N = total − items.length`다.
- 예: 전체 15건, 표시 4건이면 ‘외 11개 스터디’다. 4건 이하이면 표시하지 않는다.
- 모집중 카드가 전체 8건·표시 4건이면 하단 중앙에 ‘외 4개 스터디’를 표시한다. 이 문구는 별도의 다섯 번째 스터디 항목이 아니라 전체 목록으로 이동하는 링크다.
- **클릭하면 대시보드에서 해당 조건의 전체 스터디 목록 화면으로 이동한다.** 카드 안에 나머지 항목을 펼치는 동작이 아니다. 해당 영역 위쪽의 ‘전체 보기’와 같은 동작이다.
- 예: 진행중 카드에서 ‘외 11개 스터디’를 누르면 **진행중 조건이 적용된 전체 목록**으로 이동한다. 목록의 전체 대상은 15개이며, 대시보드에서 보지 못한 11개만 보여주는 것이 아니다. 모집중 카드에서는 동일하게 모집 보드의 조건을 유지한 전체 목록으로 이동한다.
- 목록으로 이동한다는 동작은 결정되었다. 실제 페이지 주소·필터 연결은 **TBD-07**, 마감된 항목을 모집 목록에 남기는 범위는 **TBD-02**에서 확정한다.
...
- 지역별 인원, 비율, 도넛 차트를 함께 표시한다. 비율은 `지역 인원 / 활성 크루 수 × 100`이다. 반올림한 비율의 합이 99% 또는 101%가 되는 것은 허용하되, **지역 API 응답 안의 인원 합은 그 응답의 활성 크루 수(totalCrew)와 반드시 일치해야 한다.** 상단 활성 크루 카드와도 같은 참여자 데이터를 읽었다면 같아야 한다. 별도 조회 사이에 회원 참여 상태가 바뀐 경우의 일시적 차이는 ‘조회 시점과 시간’ 절의 예시를 따른다.
- MVP의 지역은 **한국·북미**다. 범례는 ‘한국 KST’ → ‘북미 ET·PT’ 순서로 표시하고, 짧은 안내 ‘온보딩 시간대 기준’을 제공한다. 북미는 동부·서부를 포함하므로 `PT`만으로 표시하지 않는다.
- 아래 다섯 시간대 외의 값, 비어 있는 값, 유효하지 않은 값은 **미분류**로 집계한다. 이는 거주 지역을 이 분류표로 판단할 수 없다는 뜻이며, 별도의 지리적 지역인 ‘기타’를 뜻하지 않는다. MVP에서 전 세계 시간대 분류표를 만들거나 ‘기타’ 지역을 추가하지 않는다.
- 미분류 회원도 활성 크루 총인원과 비율의 분모에 포함한다. 1명 이상이면 도넛 차트에 포함하고 범례 마지막에 ‘미분류’와 인원·비율을 표시한다. 시간대 보조 라벨은 붙이지 않는다. 미분류가 0명이면 해당 차트 조각과 범례를 숨긴다.
- 예: 한국 1명·북미 1명·미분류 1명이면 총 3명이며, 각 비율은 `1 / 3 × 100`이다. 미분류를 빼고 한국·북미를 각각 50%로 표시하지 않는다.
- 활성 크루가 없으면 API의 한국·북미·미분류 인원은 모두 0, 비율은 `null`로 전달한다. 차트·범례 대신 ‘활성 크루가 없습니다.’를 표시한다.

**확정한 시간대 분류표**

| 저장된 시간대 | 대시보드 분류 | 비고 |
|---|---|---|
| `Asia/Seoul` | 한국 (`KOREA`) | 서울 |
| `America/New_York`, `America/Toronto` | 북미 (`NORTH_AMERICA`) | 뉴욕·토론토 |
| `America/Vancouver`, `America/Los_Angeles` | 북미 (`NORTH_AMERICA`) | 밴쿠버·LA |
| 위 다섯 값 외의 모든 값, `null`·빈 값·유효하지 않은 값 | 미분류 (`UNKNOWN`) | 유효한 다른 시간대여도 임의로 지역을 추정하지 않음. 집계 대상에서 제외하지 않음 |

현재 playground의 온보딩 선택지는 다섯 도시를 **세 항목**으로 묶는다. ‘한국·서울’은 `Asia/Seoul`, ‘북미 동부·뉴욕, 토론토’는 `America/New_York`, ‘북미 서부·밴쿠버, LA’는 `America/Vancouver` 값을 사용한다. 위 분류표는 `America/Toronto`·`America/Los_Angeles`가 저장되어 있는 경우도 북미로 분류한다. 대시보드는 저장된 시간대를 읽어 분류하며 원래 값을 다른 시간대로 바꾸지 않는다.

**출처:** [Account.timeZone](../../backend/domain/src/main/java/com/studyclub/domain/account/Account.java), 작성자 결정, 아래 근거 표의 beta `d2a00ed` 온보딩 선택지·서버 검증 코드. `REGION_GROUP` 컬럼이 존재한다는 이유만으로 온보딩 시간대와 동기화되어 있다고 가정하지 않는다.

**미정 — [NEEDS CLARIFICATION] TBD-06:** 남은 확인 사항은 **온보딩 서버가 저장을 허용할 시간대 범위**다. 확인한 서버 검증은 위 세 선택지나 다섯 시간대로 제한하지 않고 유효한 `ZoneId`를 허용한다. 실제 DB에 다른 시간대가 저장되어 있다는 뜻은 아니다. 온보딩 기획·백엔드에서 허용값을 제한할지 확인한다. 대시보드의 분류표·미분류 처리·범례는 위 내용으로 결정되었으며, 이 확인 전에도 표 밖의 값은 미분류로 처리할 수 있다.
...
**설명:** 네 KPI와 주간 비교를 한 응답에서 제공하되, 특정 집계의 실패로 다른 정상 지표까지 버리지 않는다. 인증·파라미터·본문·오류는 위 공통 계약을 따른다. Query Parameter는 없다. 사용처는 1-1·2다.

**응답 표현 제안 — 필드 구조는 TBD-07에서 개발 검토 후 확정**

요약 API는 유지하고 아래 집계 묶음별 성공·실패를 표현하는 안이다. 일부 묶음만 실패하면 HTTP 200으로 정상 지표와 실패 정보를 함께 반환한다. 전체 집계가 실패해 제공할 수 있는 지표가 없으면 공통 500 응답을 사용한다. HTTP 200만 보고 모든 카드가 성공했다고 판단하지 않는다.

| 집계 묶음 | 함께 반환할 필드 | 실패 시 영향을 받는 카드 |
|---|---|---|
| `participation` | activeCrew, activeEnrollments, studiesPerCrew | 2-1·2-3. 같은 활성 참여 집합으로 계산 |
| `attendance` | attendanceRate, attendanceComparison | 2-2·2-2-1. 같은 유효 출석 규칙으로 계산 |
| `communityMembers` | communityMembers | 2-4 |

실패한 묶음은 `errors`에 오류 정보를 넣고 그 묶음의 데이터 필드를 **생략**한다. 실패를 `null`로 보내 유효 기록 없음과 혼동하지 않는다. 성공한 묶음은 모든 필드를 제공하며, 산식상 계산할 값이 없을 때만 아래 표에서 허용한 `null`을 사용한다. 모든 집계가 성공하면 `errors`는 빈 객체다. 성공·실패 정보와 필드가 서로 맞지 않는 응답을 0이나 정상 결과로 보정하지 않는다.

**Response — 200 (모든 집계 성공)**
...
이 예에서는 평균 출석률·전주 대비만 오류로 표시하고 다른 KPI는 정상 표시한다. 이전 출석 데이터가 있으면 ‘갱신 실패 · 이전 데이터’로 구분해서 유지할 수 있다. 새 응답의 `asOf`를 이전 출석 데이터의 갱신 시각으로 덮어쓰지 않는다.

아래 NULL 표시는 **해당 집계가 성공하여 필드가 제공될 때**의 허용 여부다. 집계 실패 시 필드 생략은 위 규칙을 따른다. `asOf`, `timeZone`, `errors`는 모든 200 응답에 포함한다.
...
| errors.{집계 묶음}.errorCode | String | N | 해당 묶음의 집계 실패 코드. 제안 값 INTERNAL_ERROR |
| errors.{집계 묶음}.errorMessage | String | N | 사용자에게 공개 가능한 실패 설명. 내부 예외·쿼리 등은 포함하지 않음 |

**미확정:** TBD-01(진행 중), TBD-03(유효 출석), TBD-07(성공·실패 응답의 구체적인 필드·재시도 연결 등 최종 계약). 정상 지표 유지와 의존 지표의 동반 오류라는 화면 규칙은 결정되었다. API의 필드 이름과 보조 필드는 제안이다.
...
`ongoing`과 `recruiting`은 서로 하나만 선택되어야 하는 기수 상태가 아니라 **조회할 목록의 조건**이다. 진행하면서 모집하는 기수는 두 요청의 대상에 모두 포함될 수 있다. 각 요청의 total은 그 조건의 고유 기수 수로 세고, 미리보기 items는 각 조건의 정렬·limit을 적용한다.
...
| items[].deadlineBadge | Enum | Y | 계산: 3-2 우선순위. `DEADLINE_PASSED / TODAY / CLOSING_SOON / NORMAL / ALWAYS_OPEN`. 오른쪽 마감 텍스트의 문구·색상을 선택하는 상태 코드이며 배경색 배지를 뜻하지 않음. ongoing 뷰는 null |
...
진행중 응답은 `view=ONGOING`, 동일한 기수 식별·이름 구조를 사용하며 마감 상태 코드·남은 일수·경과 사유는 null로 전달하고 화면에도 표시하지 않는다. 총 건수가 0이면 `total=0`, `items=[]`다. 모집중 응답에서는 날짜를 KST `MM-DD`로 변환해 `~MM-DD (D-n)` 형식으로 표시한다. 위 응답 예시의 `CLOSING_SOON`은 `~09-19 (D-3 · 임박)`으로 표시한다. 수동 마감이면서 마감일은 미래인 항목은 남은 일수가 양수여도 **마감 상태 우선**으로 `~MM-DD (마감 경과)`를 표시하고, 마감일이 없으면 ‘마감 경과’만 표시한다.
...
| regions[].code | Enum | N | 계산: ACCOUNT.TIME_ZONE을 화면 4의 확정 분류표로 분류. 코드명 제안은 `KOREA / NORTH_AMERICA / UNKNOWN` |
...
`totalCrew`가 0이면 세 항목의 `count`는 모두 0, `percentage`는 모두 `null`이다. 미분류가 존재하면 한국·북미와 함께 인원 합계 및 비율 계산에 포함한다. 별도의 `OTHER` 항목은 반환하지 않는다.

**미확정:** TBD-01(활성 모수의 진행 상태), TBD-07(최종 응답 계약). TBD-06은 온보딩 서버의 허용 시간대 범위 확인이며, 대시보드의 분류표·미분류 처리·범례는 확정되었다.
...
| 모집 | null 마감 지원은 beta 반영. 기존 임박 기준은 3일 | 이 화면의 7일 임박 기준과 마감 표시 형식 적용. 수동 마감·보드 대상은 TBD-02 |
...
| 지역 | mock 집계는 스터디별 순회 중 같은 사람을 중복 집계할 수 있음. beta d2a00ed의 온보딩 UI는 세 선택지지만 서버는 다른 유효 시간대도 허용 | 회원 중복 제거 후 확정한 다섯 시간대를 한국·북미로 분류하고 나머지는 미분류 처리. 미분류도 totalCrew와 비율 분모에 포함하며, 0명일 때만 미분류 범례를 숨김. 서버 허용 범위는 TBD-06에서 확인 |
...
| 오류 | playground의 위젯별 로딩·오류 상태 미구현 | 정상 지표는 유지하고 실패·의존 지표만 오류 표시. summary의 부분 성공 응답을 연결하며 구체적인 필드 계약은 TBD-07에서 검토 |
...
| 같은 활성 크루가 여러 기수에 참여 | 지역 분포에 한 번만 포함. 지역 인원 합계 = 해당 지역 API의 totalCrew. 같은 데이터를 읽은 활성 크루 카드와도 일치 |
| 서울·뉴욕·토론토·밴쿠버·LA 시간대의 활성 크루가 각각 1명 | 한국 1명·북미 4명, 총 5명. 범례는 한국 KST·북미 ET·PT. ‘기타’ 항목 없음 |
| 다섯 시간대 밖의 유효한 값 또는 null·빈 값·잘못된 값 | 모두 미분류. 회원을 누락하거나 임의로 한국·북미에 배정하지 않음 |
| 한국 1명·북미 1명·미분류 1명 | totalCrew 3명, 각 비율 1/3 × 100. 미분류도 도넛·범례에 인원·비율 표시. 미분류에 시간대 보조 라벨 없음 |
| 미분류가 0명 | API는 UNKNOWN count 0을 반환하고 화면은 해당 차트 조각·범례를 숨김 |
| 활성 크루가 0명 | 세 분류의 count 0·percentage null. 차트·범례 대신 ‘활성 크루가 없습니다.’ 표시 |
| 활성 크루 카드 조회 후 참여자 등록, 이후 지역 차트 조회 | 두 응답 사이의 실제 데이터 변경으로 생긴 일시적 차이와 중복·누락 집계 오류를 구분. 각 응답 내부의 계산은 일치 |
| 마감 시각과 asOf가 같음 | 마감 경과. 아직 오늘이라는 이유로 오늘 마감 표시하지 않음 |
| 마감일은 미래 또는 null인데 수동 마감 | 마감 경과. 미래 날짜의 D-day·상시 문구가 이를 덮어쓰지 않음 |
| 마감까지 7일 / 8일 / 마감일 없음 | 각각 임박 / 일반 / 상시. 수동 마감은 우선 적용 |
| 모집중 카드에 전체 8건 | 상단 왼쪽 ‘모집중 스터디 8’, 오른쪽 ‘전체 보기’, 본문 최대 4개 행, 하단 중앙 ‘외 4개 스터디’ 표시. 두 목록 링크 모두 같은 전체 8건을 대상으로 이동 |
| 모집중 행의 이름이 길고 마감일은 16일 뒤 | 왼쪽 이름은 한 줄 말줄임, 오른쪽은 `~MM-DD (D-16)` 한 줄 오른쪽 정렬. 예를 들어 기준일 09-13·마감일 09-29면 `~09-29 (D-16)` |
| 마감일 당일 / 기한 경과 / 마감 3일 전 | 같은 오른쪽 영역에 각각 `~MM-DD (오늘 마감)` / `~MM-DD (마감 경과)` / `~MM-DD (D-3 · 임박)`. 색과 문구를 함께 구분 |
| 진행 중이면서 실제로 모집도 하고 있는 기수 | 두 보드의 전체 대상·건수에 각각 포함. 각 미리보기는 정렬·최대 4건 적용. 한쪽 대상에서 임의 제외하지 않음 |
| 현황 보드 진입 | 진행중·모집중·예정 행사 세 영역을 동시에 확인할 수 있음. 특정 탭을 눌러야 다른 목록이 보이는 구조가 아님 |
| 진행중 기수 15개에서 ‘외 11개 스터디’ 클릭 | 대시보드는 미리보기 4개·외 11개 표시. 클릭하면 진행중 전체 15개를 대상으로 하는 목록 화면으로 이동. 카드 안에서 펼치거나 남은 11개만 조회하지 않음 |
...
| summary에서 출석 집계만 실패 | 평균 출석률·전주 대비에만 오류 표시. 같은 응답의 활성 크루·1인당 참여 스터디·커뮤니티 멤버는 정상 표시 |
| summary에서 활성 참여자 집계 실패 | 활성 크루·1인당 참여 스터디 모두 오류 표시. 정상 출석률·커뮤니티 멤버는 유지. 실패한 분모를 0으로 간주해 계산하지 않음 |
| summary에서 커뮤니티 멤버 집계만 실패 | 커뮤니티 멤버 카드만 오류 표시. 나머지 정상 KPI는 유지 |
| 출석 집계 성공이나 유효 기록 없음 / 출석 집계 실패 | 각각 성공 데이터의 null로 빈 상태 표시 / 별도 실패 정보로 오류 표시. 둘을 혼동하지 않음 |
| summary 요청 자체의 통신 오류 또는 전체 집계 실패 | KPI 네 개에 오류 표시. 별도 요청으로 성공한 목록·차트는 유지 |
| 부분 실패 후 다시 시도 | 기존 정상 카드를 비우지 않음. 새 성공 데이터 적용 시 활성 크루·참여 건수·1인당 참여 수는 같은 응답으로 함께 갱신 |
| 재시도에서도 특정 지표 실패, 해당 지표의 이전 데이터 있음 | 이전 값·이전 갱신 시각과 갱신 실패 안내 유지. 새 asOf를 이전 데이터에 적용하지 않음 |
...
| 이 문서에 포함한 기준 화면 및 작성자의 추가 결정(2026-09-16 및 후속 검토) | 활성·참여 정의, PAUSED 제외, 시간대 가정과 한국·북미 매핑 및 미분류 처리, 마감 경과, 진행 중 기수의 동시 모집 허용, 주간 비교, 평균 계산, 행사 범위, 일부 지표 실패 시 정상 지표 유지 |
| [온보딩 시간대 선택지](https://github.com/StudyClub-PlusPlus/studyclub-engineering/blob/d2a00eda87b4139cca037d391becbb3b2c74b3c7/frontend/apps/playground/src/proto/core/components/TimeZonePicker.tsx) | 지역 분포 후속 검토 시 beta d2a00ed에서 확인. 다섯 도시를 세 선택지로 표시하며 값은 Asia/Seoul·America/New_York·America/Vancouver |
| [온보딩 시간대 서버 검증](https://github.com/StudyClub-PlusPlus/studyclub-engineering/blob/d2a00eda87b4139cca037d391becbb3b2c74b3c7/backend/api/src/main/java/com/studyclub/api/auth/validation/TimeZoneValidator.java) | 같은 beta에서 확인. 유효한 ZoneId를 허용하며 UI 선택지로 제한하지 않음. 실제 DB에 다른 값이 존재하는지는 확인하지 않음 |
...
| [PR #89에서 확인한 신청서 명세](https://github.com/StudyClub-PlusPlus/studyclub-engineering/blob/ab78e92219d27e3e977e653d8cda8e9c8d1d1f4c/docs/erd/STUDY_APPLICATION.md) | 신청 제출과 명부 등록을 함께 처리하는 변경안. 이 명세만으로 실제 신청 서비스의 구현·배포 완료를 뜻하지는 않음 |
...
|---|---|---|---|---|
| **TBD-01** | 2-1, 2-3, 3-1, 3-2, 4, 6 | 진행 여부를 최종 DB·API에서 확인할 상태값 또는 날짜 조건, 기수 ID·표시명 원천, 신청 기능의 실제 명부 등록 연결 | 초안에서 확인한 저장 상태는 DRAFT/OPEN/CLOSED이며 OPEN은 공개됐다는 뜻이라 진행 중과 같지 않음. 상태·테이블 개편과 신청 기능의 최종 구현을 확인해야 함. ‘명부 등록’은 신청자를 실제 참여자 목록에 넣는 처리이며 PR #89는 제출 시 등록하는 방향 | 스터디 기획·백엔드 / 결정된 집계 기준을 최종 상태·날짜·참여 명부 데이터에 연결. **진행 중에도 모집할 수 있고 두 목록에 함께 포함한다는 점은 확정**. 기수별 1개 및 ACTIVE만 센다는 결정도 유지 |
...
| **TBD-06** | 4 / 온보딩 연계 | 온보딩 서버가 저장을 허용할 시간대 범위를 UI 선택지 등에 맞춰 제한할지 | beta d2a00ed의 UI는 세 선택지지만 서버는 다른 유효 ZoneId도 허용. 실제 DB에 다른 값이 저장되어 있다는 뜻은 아님 | 온보딩 기획·백엔드 / 서버 허용값 정책 확인. **다섯 시간대의 한국·북미 분류, 나머지 값의 미분류 처리, 미분류 0명 숨김, 범례·보조 라벨은 확정**. 확인 전에도 대시보드는 미분류 규칙으로 집계 가능 |
| **TBD-07** | 1-1, 2 전체, 3의 링크, 6의 링크, 전체 API | 정상·실패 지표를 구분하는 응답 필드와 재시도 연결, 실제 목록·상세 경로와 필터, 제안한 보조 문구의 최종 합의 | 정상 지표 유지·같은 데이터에 의존하는 지표의 동반 오류는 결정됨. summary 하나로 부분 성공을 반환하는 방향이며, 문서의 집계 묶음·errors 필드는 개발 검토용 제안. 현재 playground 링크는 기수·카테고리·회원 범위와 어긋날 수 있고 지역 API도 추가 제안 상태 | 프론트·백엔드, 문구는 기획 / 결정된 오류 표시 규칙에 맞춰 응답·재시도 계약 검토 및 같은 대상을 여는 링크 합의. 커뮤니티 멤버 보조 문구 ‘온보딩 완료 회원’ 등 제안 검토 |
