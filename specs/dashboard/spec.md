...
- **작성 기준 확정**: 기수 단위 집계, 활성 크루의 범위, 온보딩 완료 회원 수, 시간대를 거주지로 간주하는 가정, 출석 계산 및 비교 기간, 마감 경과 표시, 진행 중 기수의 동시 모집 허용, 행사 MVP 제외, 일부 지표 실패 시 정상 지표 유지.
...
**모집중 카드는 아래 구성으로 표시한다.** 아래의 8건·스터디 이름·날짜·D-day는 표시 형식을 설명하는 예시이며, 실제 숫자는 조회 결과와 집계 기준 시각으로 계산한다.
...
| 위치 | 표시·동작 |
|---|---|
| 상단 왼쪽 | 굵은 제목 ‘모집중 스터디’와 제목보다 작은 숫자로 전체 대상 건수 표시. 예시의 `8`은 카드에 보이는 4건을 포함한 전체 8건 |
| 상단 오른쪽 | 파란색 ‘전체 보기’ 링크. 모집 보드와 같은 조건의 전체 스터디 목록 화면으로 이동 |
| 본문 각 행 왼쪽 | 기수를 구분할 수 있는 스터디 이름을 굵게 한 줄로 표시. 길면 `System Design ...`처럼 말줄임 처리하고 전체 이름은 툴팁·접근 가능한 이름으로 제공 |
| 본문 각 행 오른쪽 | 마감일과 남은 일수를 `~MM-DD (D-n)` 형식으로 오른쪽 정렬. 예: `~09-29 (D-16)`. 날짜 앞의 `~`는 ‘이 날짜까지 모집’이라는 뜻 |
| 행 전체 | 클릭하면 해당 기수의 상세 화면으로 이동. 이름과 마감 정보가 같은 행에 있으며, 마감 정보를 읽을 공간을 확보하고 긴 이름 쪽을 말줄임 처리 |
| 하단 중앙 | 전체가 4건을 넘으면 회색 글자로 ‘외 N개 스터디’를 표시. 전체 8건 중 4건을 보여주면 ‘외 4개 스터디’. 클릭 시 상단 ‘전체 보기’와 같은 목록으로 이동 |
...
| 이 문서에 포함한 기준 화면 및 작성자의 추가 결정(2026-09-16 및 후속 검토) | 활성·참여 정의, PAUSED 제외, 시간대 가정, 마감 경과, 진행 중 기수의 동시 모집 허용, 주간 비교, 평균 계산, 행사 범위, 일부 지표 실패 시 정상 지표 유지 |
